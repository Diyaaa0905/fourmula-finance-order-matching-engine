...
class CircuitBreaker {
    ...
    void check(Stock stock, OrderLog log) {
        ...
        double deviation = Math.abs(stock.lastTradedPrice - stock.openPrice)
                / stock.openPrice;
...
class Stock {
    ...
    Stock(String ticker, double tickSize) {
        ...
        this.circuitBreaker = new CircuitBreaker(5.0);
    }
...
class Exchange {
    ...
    boolean isValidTicker(String t) {
        return stocks.containsKey(t);
    }

    Stock getStock(String t) {
        return stocks.get(t);
    }
...
class TradeRecord {
    ...
    final String buyTraderId, sellTraderId;
    final String ticker;
    final double price;
    final int quantity;
    final long timestamp;

    TradeRecord(Order buy, Order sell, double price,
                int qty, String ticker) {
    ...
    public String toString() {
        return String.format(
                "Trade[%s | %s | buyer=%s | seller=%s | $%.2f | qty=%d]",
                tradeId,
                ticker,
                buyTraderId,
                sellTraderId,
...
class RingBuffer {
    ...
    List<Order> toList() {
        ...
        for (int i = 0; i < size; i++)
            list.add(buffer[(head + i) % capacity]);
...
class OrderBook {
    final TreeMap<Double, PriceLevel> bids =
            new TreeMap<>(Collections.reverseOrder());

    final TreeMap<Double, PriceLevel> asks =
            new TreeMap<>();
    ...
    int availableQuantity(Order incoming) {
        ...
        boolean isMarket =
                (incoming.type == OrderType.MARKET_FOK
                        || incoming.type == OrderType.MARKET);

        int total = 0;

        for (Map.Entry<Double, PriceLevel> entry
                : opposite.entrySet()) {
            ...
            for (Order r : entry.getValue().queue.toList()) {
                ...
                if (!r.traderId.equals(incoming.traderId))
                    total += r.quantity;
            }
...
class StopOrderBook {
    final TreeMap<Double, List<Order>> stopLoss =
            new TreeMap<>(Collections.reverseOrder());

    final TreeMap<Double, List<Order>> stopBuy =
            new TreeMap<>();
    ...
    List<Order> getTriggeredOrders(double marketPrice) {
        ...
        for (Map.Entry<Double, List<Order>> e
                : stopLoss.entrySet()) {

            if (marketPrice <= e.getKey())
                triggered.addAll(e.getValue());
        }
        ...
        stopLoss.entrySet()
                .removeIf(e -> triggered.containsAll(e.getValue()));
...
class MatchingEngine {
    ...
    void process(Order order, Stock stock) {
        ...
        if (stock.tradingPaused
            ...
            orderLog.record(
                    order,
                    "REJECTED: Circuit Breaker active. Trading is paused."
            );
        ...
        switch (order.type) {
            ...
            case MARKET:
                handleMarketNormal(order, stock);
                break;

            case MARKET_IOC:
                handleMarketIOC(order, stock);
                break;

            case MARKET_FOK:
                handleMarketFOK(order, stock);
                break;

            case LIMIT:
                handleLimitNormal(order, stock);
                break;

            case LIMIT_IOC:
                handleLimitIOC(order, stock);
                break;

            case LIMIT_FOK:
                handleLimitFOK(order, stock);
                break;
    ...
    private void handleStop(Order order) {
        ...
        orderLog.record(
                order,
                "Stored in stop book at threshold: "
                        + order.price
        );
    }
    ...
    private void handleMarketNormal(Order order, Stock stock) {
        ...
        if (order.quantity > 0)
            orderLog.record(
                    order,
                    "Partial fill - remainder added to book"
            );
        else
            orderLog.record(order, "Fully filled");
    }
    ...
    private void handleMarketIOC(Order order, Stock stock) {
        ...
        if (order.quantity > 0) {
            ...
            orderLog.record(
                    order,
                    "IOC - unfilled remainder cancelled"
            );
    ...
    private void handleMarketFOK(Order order, Stock stock) {
        ...
        int available =
                orderBook.availableQuantity(order);
    ...
    private void handleLimitNormal(Order order, Stock stock) {
        ...
        if (order.quantity > 0)
            orderLog.record(
                    order,
                    "Partial/no fill - remainder added to book"
            );
        else
            orderLog.record(order, "Fully filled");
    }
    ...
    private void handleLimitIOC(Order order, Stock stock) {
        ...
        if (order.quantity > 0) {
            ...
            orderLog.record(
                    order,
                    "Limit IOC - unfilled remainder cancelled"
            );
    ...
    private void handleLimitFOK(Order order, Stock stock) {
        ...
        if (available < order.quantity) {
            ...
            orderLog.record(
                    order,
                    "Limit FOK failed - only "
                            + available
                            + " available at limit, needed "
                            + order.quantity
            );
    ...
    private void match(
        ...
        while (incoming.quantity > 0
            ...
            Map.Entry<Double, PriceLevel> bestEntry =
                    opposite.firstEntry();
            ...
            while (!level.isEmpty()
                ...
                UserProfile buyerProfile =
                        um.getUser(buy.traderId);

                UserProfile sellerProfile =
                        um.getUser(sell.traderId);

                double tradeValue =
                        bestPrice * tradeQty;

                double fee =
                        tradeValue * exchange.FEE_RATE;

                exchange.collectFee(fee * 2);

                buyerProfile.settleBuy(
                        stock.ticker,
                        tradeQty,
                        bestPrice,
                        fee
    ...
    private void checkStopTriggers(Stock stock) {
        ...
        List<Order> triggered =
                stopOrderBook.getTriggeredOrders(
...
class UserProfile {
    ...
    int getShares(String t) {
        ...
        return portfolio.containsKey(t)
                ? portfolio.get(t).shares
                : 0;
    }
    ...
    public String toString() {
        ...
        return String.format(
                "User[%s | %s | Bal: $%.2f | P&L: $%.2f | Portfolio: %s]",
                traderId,
                name,
                balance,
                realizedPnL,
...
class UserProfileManager {
    ...
    void addUser(UserProfile u) {
        users.put(u.traderId, u);
    }
...
class OrderValidator {
    ...
    static String validate(
        ...
        if (!um.userExists(order.traderId))
            return "Invalid trader: "
                    + order.traderId;

        if (!ex.isValidTicker(order.ticker))
            return "Invalid ticker: "
                    + order.ticker;

        Stock stock =
                ex.getStock(order.ticker);

        if (stock.tradingPaused
                && order.type != OrderType.STOP_LOSS
                && order.type != OrderType.STOP_BUY)

            return "Trading paused for: "
                    + order.ticker;
        ...
        boolean needsPrice =
                (order.type == OrderType.LIMIT
                        || order.type == OrderType.LIMIT_IOC
                        || order.type == OrderType.LIMIT_FOK
                        || order.type == OrderType.STOP_LOSS
                        || order.type == OrderType.STOP_BUY);
        ...
        if (needsPrice) {
            ...
            if (rem > EPSILON
                ...
                return "Price violates tick size: "
                        + stock.tickSize;
        }

        UserProfile user =
                um.getUser(order.traderId);
        ...
        if (order.side == OrderSide.SELL) {
            ...
            if (user.getShares(order.ticker)
                    < order.quantity)
 ...
 * NEW:
 *
 * This is now a thread-safe FIFO queue.
 *
 * Multiple producer threads can call enqueue().
 * One matching worker calls dequeue().
 */
class IncomingOrderQueue {
    ...
    Order dequeue() throws InterruptedException {
        return queue.take();
    }
 ...
 * NEW:
 *
 * There is exactly ONE matching worker.
 *
 * It repeatedly:
 *
 *     take first order
...
class MatchingWorker implements Runnable {
    ...
    private final IncomingOrderQueue queue;
...
public class Main {
     ...
     * NEW TEST:
     *
     * Creates three producer/client threads.
     *
     * All three can enqueue orders concurrently.
     *
     * The single MatchingWorker processes them
     * one at a time.
     */
    static void runConcurrentOrderTest(
            IncomingOrderQueue queue,
            UserProfileManager um,
            Exchange exchange,
        ...
        try {
            ...
            /*
             * The producer threads have finished submitting.
             *
             * Now wait until the worker has also finished
             * processing every queued order.
             */
            while (!queue.isEmpty()
                    || worker.isProcessing()) {
            ...
            System.out.println(
                    "=== CONCURRENT ORDER TEST COMPLETE ===\n"
            );
    ...
    /*
     * Creates one simulated client thread.
     *
     * IMPORTANT:
     *
     * This thread does NOT call engine.process().
     *
     * It only:
     *
     *     create order
     ...
     *     enqueue
     */
    static Thread createTestClient(
            String clientName,
            String orderId,
            String traderId,
            OrderSide side,
        ...
        return new Thread(() -> {
            Order order = new Order(orderId, side, price, quantity, OrderType.LIMIT, traderId, "AAPL");

            String validation = OrderValidator.validate(order, um, exchange);

            if (validation.equals("VALID")) {
                System.out.println("[" + clientName + "] Enqueuing " + order.id);
                queue.enqueue(order);

            } else {
                System.out.println("[" + clientName + "] REJECTED: " + validation);
            }
        }, clientName);
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                "=== LOADING PREDEFINED ORDERS ==="
        );

        /*
         * Keep the initial setup synchronous.
         *
         * These aren't incoming concurrent orders.
         * They simply establish the initial book state.
         */
            ...
            try {
                ...
                switch (cmd) {
                    ...
                    case "HELP":
                        ...
                        System.out.println(
                                "  BUY <traderId> <ticker> <qty> <price> <type>"
                                        + " - Submit a BUY order"
                        );

                        System.out.println(
                                "  SELL <traderId> <ticker> <qty> <price> <type>"
                                        + " - Submit a SELL order"
                        );

                        System.out.println(
                                "      (Available Types: LIMIT, MARKET, "
                                        + "LIMIT_IOC, MARKET_IOC, STOP_LOSS, etc.)"
                        );

                        System.out.println(
                                "      (Note: For MARKET orders, set price to 0)"
                        );
                        ...
                        System.out.println(
                                "  TAPE      - Show the Trade Tape (History)"
                        );

                        System.out.println(
                                "  PROFILES  - Show all User Profiles and P&L"
                        );

                        System.out.println(
                                "  STATS     - Show Market Variables & Revenue"
                        );

                        System.out.println(
                                "  RESUME    - Clear Circuit Breaker and resume trading"
                        );

                        System.out.println(
                                "  TEST      - Run concurrent producer/queue test"
                        );
                    ...
                    case "SELL":
                         ...
                         * NEW BEHAVIOUR:
                         *
                         * The console thread validates and enqueues.
                         *
                         * It does NOT call engine.process().
                         *
                         * The MatchingWorker does that.
                         */
                    ...
                    case "STATS":
                        ...
                        System.out.printf(
                                "Trading Paused (Circuit Breaker): %b%n",
                                s.tradingPaused
                        );

                        System.out.printf(
                                "Exchange Revenue Collected: $%.2f%n",
                                exchange.totalRevenue
                        );
                ...
                System.out.println(
                        "Input Error: Invalid order type "
                                + "or parameter. Please check your spelling."
                );
         ...
         * Clean shutdown.
         *
         * First allow any remaining queued orders
         * to finish.
         */
        try {
